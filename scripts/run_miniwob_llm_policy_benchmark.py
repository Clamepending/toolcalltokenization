...
from toolcalltokenization.llm_client import load_api_key
from toolcalltokenization.miniwob_benchmark import (
    build_group_registry,
    default_miniwob_url,
    evaluate_live_llm_policy_benchmark,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a live MiniWoB LLM macro-policy benchmark from saved traces and episodes."
    )
    ...
    parser.add_argument("--action-scope", choices=("task", "global"), default="task", help="Whether the LLM sees only task-local macros or one shared global action space.")
    ...
    parser.add_argument("--model", default="gpt-4.1-mini", help="OpenAI-compatible chat model used for action selection.")
    ...
    parser.add_argument("--cache-path", default="outputs/cache/miniwob_llm_choices.jsonl", help="Prompt-response cache JSONL path.")
...
def main() -> None:
    ...
    benchmark = evaluate_live_llm_policy_benchmark(
        rows,
        episodes,
        registry,
        group_by=args.group_by,
    ...
    dump_json(str(output_prefix.with_name(output_prefix.name + "_llm_policy_benchmark.json")), benchmark)
