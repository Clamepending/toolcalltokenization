...
from toolcalltokenization.miniwob_benchmark import (
    ...
    default_miniwob_url,
    evaluate_live_learned_policy_benchmark,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a live MiniWoB learned macro-policy benchmark from saved traces and episodes."
    )
    ...
    parser.add_argument("--train-ratio", type=float, default=0.8, help="Fraction of episodes used for mining and selector training.")
    ...
    parser.add_argument("--action-scope", choices=("task", "global"), default="task", help="Whether the learned policy sees only task-local macros or one shared global action space.")
    parser.add_argument("--no-start-step-guard", action="store_true", help="Disable the first-step structural compatibility guard.")
    parser.add_argument("--training-epochs", type=int, default=8, help="Number of perceptron passes over the live training contexts.")
    parser.add_argument("--training-seed", type=int, default=0, help="Random seed for perceptron training order.")
...
def main() -> None:
    ...
    benchmark = evaluate_live_learned_policy_benchmark(
        rows,
        episodes,
        registry,
        group_by=args.group_by,
    ...
    dump_json(str(output_prefix.with_name(output_prefix.name + "_learned_policy_benchmark.json")), benchmark)
